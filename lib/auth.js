...
async function loadUserFromToken(token) {
  ...
  // link-pending gate (plan 5.1.3): a minor student with no active family link is held.
  if (u.role === 'student' && !u.is_adult_student) {
    const { rows: fl } = await query(`SELECT 1 FROM family_links WHERE student_id = $1 AND status = 'active' LIMIT 1`, [u.id]);
    u.link_pending = fl.length === 0;
  } else {
    u.link_pending = false;
  }
  return u;
}
...
function authRequired(optional = false) {
  return async (req, res, next) => {
    ...
    try {
      ...
      req.user = user;
      // central link-pending enforcement (plan 5.1.3): a held minor gets nothing but /api/me,
      // so no content route or side effect (e.g. progress auto-insert) runs for them.
      if (user.role === 'student' && user.link_pending && req.path !== '/api/me') {
        return res.status(403).json({ error: 'link pending', link_pending: true });
      }
      next();
    } catch (err) {
...
function requireRole(...roles) {
  return (req, res, next) => {
    ...
    // A minor with no active parent link is held at the link-pending screen (plan 5.1.3).
    if (req.user.role === 'student' && req.user.link_pending) {
      return res.status(403).json({ error: 'link pending', link_pending: true });
    }
    next();
  };
